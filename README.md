...
GitHub Actions (평일 08:40 / 18:40 KST)
  └ python -m yujeung daily
      ...
      관문1·2 판정 → 가상 성과 기록 (인수권 마지막 날 스냅샷)
      → data/site.json + data/yujeung.sql 커밋
Netlify (레포 연결, 빌드 없음) → index.html 이 data/site.json 을 읽어 표시
```

## 준비 (한 번)
1. 레포 **Settings → Secrets and variables → Actions**: `DART_API_KEY`, `KRX_API_KEY`
...
| 명령 | 설명 |
|---|---|
| `daily [--today YYYYMMDD]` | 감지 → 일정 → 시세 → 판정·가상성과 → site.json/덤프 |
| `backfill-cases --months 5 [--dry-run] [--report 210980]` | 최근 N개월 공시를 월 단위로 조회해 진행 중 케이스 채우기 (멱등) |
| `report-case 210980` | 케이스 일정·괴리율 추이·판정·가상성과 출력 |
