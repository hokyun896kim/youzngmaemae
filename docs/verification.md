...
| DART `document.xml` 원문 → 일정 파서 | 🟡 1건 실측 | 이렘(2026-09-23 기재정정): 기준일·인수권 상장기간·청약·납입·상장·배정비율 추출 성공. 발행가 누락 버그(가격+날짜 한 행) 수정·회귀테스트 추가. 정정 공시 본문에 옛 날짜가 남는 사례 확인 → 청약일 이후 확정일 배제. **DART 원문과 값 대조 + 추가 5건 필요** |
| KRX Open API `sto/sr_bydd_trd` 신주인수권증서 일별매매정보 (2010-02-12~) | ❌ 401 | `Unauthorized API Call` — 같은 키로 stk/ksq 는 성공 → **이 서비스만 이용신청 미승인**. 승인 후 재검증 |
